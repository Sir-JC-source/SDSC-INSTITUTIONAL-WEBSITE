...
$userRole = $user['role'];        // superadmin or admin
$userCampus = $user['campus'];    // main or ibaan
...
function fixDisplayOrder($conn, $order, $campus, $excludeId = null) {
    $order = intval($order);
    $campusEsc = mysqli_real_escape_string($conn, $campus);

    if ($excludeId !== null) {
        $excludeId = intval($excludeId);
        $sql = "UPDATE main_hero_images 
                SET display_order = display_order + 1 
                WHERE display_order >= $order 
                AND campus='$campusEsc'
                AND id <> $excludeId";
    } else {
        $sql = "UPDATE main_hero_images 
                SET display_order = display_order + 1 
...
// Determine campus
// Determine campus correctly
if ($userRole === 'superadmin') {

    $campus = isset($_POST['campus']) ? $_POST['campus'] : 'main';

} else {

    // Admin accounts are locked to their campus
    $campus = $userCampus;

}

    fixDisplayOrder($conn, $order, $campus);
    ...
    mysqli_query($conn, "
        INSERT INTO main_hero_images 
        (image_path, active, display_order, campus) 
        VALUES ('$savedEsc',1,$order,'$campusEsc')
    ");
...
if ($userRole === 'superadmin') {
    $q = mysqli_query($conn,"SELECT campus FROM main_hero_images WHERE id=$id");
} else {
    $campusEsc = mysqli_real_escape_string($conn,$userCampus);
    $q = mysqli_query($conn,"SELECT campus FROM main_hero_images WHERE id=$id AND campus='$campusEsc'");
}

$r = mysqli_fetch_assoc($q);
$campus = $r['campus'] ?? $userCampus;

fixDisplayOrder($conn, $order, $campus, $id);
    ...
    if (empty($cropped) && !empty($_FILES['full_image_edit']['name'])) {
        ...
        mysqli_query($conn, "UPDATE main_hero_images SET image_path='$savedEsc', display_order=$order WHERE id=$id");
    } else if (!empty($cropped)) {
        // With crop
        $saved = saveBase64Image($cropped, $uploadDir);
        ...
        mysqli_query($conn, "UPDATE main_hero_images SET display_order=$order WHERE id=$id");
    }
...
if (isset($_GET['delete'])) {
    ...
    $q = mysqli_query($conn, "SELECT image_path FROM main_hero_images WHERE id=$id");
...
if ($userRole === 'superadmin') {
    mysqli_query($conn, "DELETE FROM main_hero_images WHERE id=$id");
} else {
    mysqli_query($conn, "DELETE FROM main_hero_images WHERE id=$id AND campus='$userCampus'");
}
    header("Location: display.php");
    exit();
}
...
if (isset($_GET['toggle'])) {
    ...
    $q = mysqli_query($conn, "SELECT active FROM main_hero_images WHERE id=$id");
    if ($q && mysqli_num_rows($q)) {
        $r = mysqli_fetch_assoc($q);
        $new = ($r['active'] == 1) ? 0 : 1;
if ($userRole === 'superadmin') {
    mysqli_query($conn, "UPDATE main_hero_images SET active=$new WHERE id=$id");
} else {
    mysqli_query($conn, "UPDATE main_hero_images SET active=$new WHERE id=$id AND campus='$userCampus'");
}
    }
    header("Location: display.php");
    exit();
...
// ======================================
if ($userRole === 'superadmin') {
    // Superadmin sees both campuses
    $images = mysqli_query($conn, "SELECT * FROM main_hero_images ORDER BY display_order ASC, id ASC");
} else {
    // Admin sees only their campus
    $campusEsc = mysqli_real_escape_string($conn, $userCampus);
    $images = mysqli_query($conn, "SELECT * FROM main_hero_images 
...
<head>
<meta charset="utf-8">
<title>Display Images Management | SDSC</title>
...
<div class="main-wrapper">
    <div class="d-flex align-items-center justify-content-between mb-3">
        <h3 class="page-title">Display Images Management</h3>
        <div>
...
if ($userRole !== 'superadmin' && $row['campus'] !== $userCampus) {
    continue;
}
...
<?php } ?>
                        ...
                        <td>
                            ...
                            <!-- Toggle (gold) -->
                            <a href="display.php?toggle=<?php echo $row['id']; ?>" class="btn-toggle me-2">Config</a>

                            <!-- Delete -->
                            <a href="display.php?delete=<?php echo $row['id']; ?>" class="btn-delete" onclick="return confirm('Delete this image?');">Delete</a>
                        </td>
                    </tr>
                        ...
                        <div class="modal-content">
                          <form method="POST" enctype="multipart/form-data" onsubmit="return true;">
                            <div class="modal-header">
                              <h5 class="modal-title">Edit Display Image</h5>
  ...
  <div class="modal-dialog modal-xl modal-dialog-centered">
    <div class="modal-content">
      <form id="addForm" method="POST" enctype="multipart/form-data" onsubmit="return true;">
        <div class="modal-header">
          <h5 class="modal-title">Add Display Image (Optional crop to 1920×700)</h5>
          <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
        </div>
...
<div class="mb-3">
<label class="small-muted">Campus</label>
